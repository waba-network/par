...
#include <waba.token.backend/waba.token.backend.wast.hpp>
#include <waba.token.backend/waba.token.backend.abi.hpp>
...
public:
   ...
   eosio_token_tester() {
      ...
      create_accounts( { N(alice), N(bob), N(carol), N(waba.token) } );
      produce_blocks( 2 );

      set_code( N(waba.token), waba_token_backend_wast );
      set_abi( N(waba.token), waba_token_backend_abi );

      produce_blocks();

      const auto& accnt = control->db().get<account_object,by_name>( N(waba.token) );
      abi_def abi;
   ...
   fc::variant get_stats( const string& symbolname )
   {
      ...
      vector<char> data = get_row_by_account( N(waba.token), symbol_code, N(stat), symbol_code );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = eosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(waba.token), acc, N(accounts), symbol_code );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( N(waba.token), N(create), mvo()
           ( "issuer", issuer)
